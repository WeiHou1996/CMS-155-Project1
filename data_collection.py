...
class Collection:    
    ...
    def parse_telecom_data(filename_train,filename_test):
        ...
        X_train_num[:,2] = Collection.parse_total_charges(X_train_num[:,2])
        X_test_num[:,2] = Collection.parse_total_charges(X_test_num[:,2])
        
        # handle contract length
        X_train_contract = X_train_in[['Contract']].to_numpy()
        X_test_contract = X_test_in[['Contract']].to_numpy()
        X_train_contract = Collection.parse_total_contract(X_train_contract)
        X_test_contract = Collection.parse_total_contract(X_test_contract)
        ...
        X_train_internetService = Collection.parse_total_internetService(X_train_internetService)
        X_test_internetService = Collection.parse_total_internetService(X_test_internetService)
...
class Collection4:    
    ...
    def parse_telecom_data(filename_train,filename_test, entry_to_delete):
        ...
        # get X with categorical data
        catData_list = ['customerID','tenure','InternetService','Contract','MonthlyCharges','TotalCharges']
        if c2drop in catData_list:
            catData_list = [i for i in catData_list if i != c2drop]
        X_train_cat = X_train_in.drop(columns=catData_list)
        X_test_cat = X_test_in.drop(columns=catData_list)
        ...
        # handle total charges
        # idx = np.where(numData_list=='TotalCharges')
        # X_train_num[:,idx] = Collection4.parse_total_charges(X_train_num[:,idx])
       #X_test_num[:,idx] = Collection4.parse_total_charges(X_test_num[:,idx])
        
        # handle contract length
        if c2drop != 'Contract':
            X_train_contract = X_train_in[['Contract']].to_numpy()
            X_test_contract = X_test_in[['Contract']].to_numpy()
            X_train_contract = Collection4.parse_total_contract(X_train_contract)
            X_test_contract = Collection4.parse_total_contract(X_test_contract)
        ...
        if c2drop != 'InternetService':
            X_train_internetService = X_train_in[['InternetService']].to_numpy()
            X_test_internetService = X_test_in[['InternetService']].to_numpy()
            X_train_internetService = Collection4.parse_total_internetService(X_train_internetService)
            X_test_internetService = Collection4.parse_total_internetService(X_test_internetService)
